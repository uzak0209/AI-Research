...
**2026-09-22 改訂: §7 のサーキットブレーカーを実装（`project_id × run_date` 単位、KV。#80）。初期閾値・半開きの要否は未決のまま**）
...
- **サーキットブレーカーの初期閾値。**実装は `project_id × run_date` を scope に KV へ連続失敗数を持ち、
  `CIRCUIT_BREAKER_THRESHOLD`（既定 3。設定が壊れていれば 1 = fail closed）で当日のその project を止める
  （#80）。復帰は「翌日（`run_date` が変わる）」のみで、同日中の半開き（half-open）による自動復帰は持たない。
  この初期値 3 と、同日中に半開きを持たせるかは実測前のため未決のまま
