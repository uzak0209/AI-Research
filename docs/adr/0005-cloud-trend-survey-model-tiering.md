...
パスを並べたのは本 ADR が初めてで、テーマ候補とファクトチェックの確定は ADR-0002 の改訂に回す。
...
**前版でこれを理由に Named Router / Routing DSL を却下したが、本版で改める。**
モデル選択をアプリのコードから追い出すこと（資料 13「自律性の第一歩」）の利得が、
設定が外に出る不利益を上回る。不利益の方は「正本をリポジトリに置く」で打ち消せる。
...
| 層 | 決定 | 意図 |
|---|---|---|
| SDK | **`max_retries=0` にする** | OpenAI SDK は既定 `max_retries=2` で、**自前のフォールバックより先に同じモデルへ再試行する**（資料 12）。cron の実行窓と Workers の実行時間を無駄に食う。再試行はルーター側に一本化する |
| ルーター | Named Router の候補内フェイルオーバー | 429 は `Retry-After` 付きで返る。**別プロバイダのモデルへ逃がすと安定する**（資料 12）ため、Allowed のベンダーを分散させる |
| 受け皿 | **候補に入れていない別ベンダー 1 本** | 資料 09-03。同一ベンダーの障害で全滅しない。**使われない限り課金は発生しない**ので、置かない理由がない |
| ストリーミング | **使わない** | OrcaRouter は「一度バイト送信が開始されると fallback は機能しない」と明記。無人バッチにストリームの利点はなく、fallback を確実に効かせる方を取る |
| レビュー段の受け皿 | **同格のみ。安価モデルへ落とさない** | 落ちた先が**捏造率を実測していないモデル**だと、その出力が黙って混入する。見送りの原因だった失敗モードが戻る |
...
| PII | **ブロックを明示設定する** | 既定は**マスク**で、ブロックは明示設定時のみ（資料 10）。無人運転では masked のまま送信が続き、気づけない。ブロックは **HTTP 400・課金なし**なので予算保護にもなる |
| PII カスタムルール | **氏名・所属機関名・共同研究者名を足す** | 既定の検出対象に**氏名・住所・社名は入らない**（資料 10）。研究概要には所属と共著者が混ざりうる。**このプロダクト固有の検出対象**であり、既定のままでは素通りする |
...
### 10. フィードバックループ ── 記録して、週次で内省する

```mermaid
flowchart LR
    run["日次 run<br/>1 段目・2 段目"] --> d1[("D1<br/>llm_calls / run_papers")]
    d1 --> scan["週次スキャン<br/>router-retro"]
    scan -->|"proposal ラベル"| iss["GitHub Issue<br/>週 3 件まで"]
...
**意図（④と①の連動）**: `.github/claude-scans/shared.md` は既に
**「決定を覆す `auto-scan` 禁止（反対は `proposal`）」**と定めている。
モデル選択・検索クエリ・段数はいずれも ADR の決定であり、機械が黙って覆してよいものではない。
また §2 で**ルーター設定の正本をリポジトリに置く**と決めたので、
自動適用するとコンソールとリポジトリが乖離する。**自律なのは実行であって、設計変更ではない。**

#### 仕組みは既存に相乗りする

- `.github/claude-scans/router-retro.md` を追加し、`claude-scan.yml` に**週次の schedule を 1 本足す**
- 新しいワークフローは作らない。既存の 3 種（design-review / adr-hygiene / research-trend）と同じ枠組み
- **日次にしない意図**: 1 日ぶんでは母数が足りず、ノイズを設計変更の根拠にしてしまう。
  shared.md の「未確認は書かない・水増し禁止」をそのまま引き継ぐ

#### 記録するもの

| 表 | 追加する列 |
|---|---|
| `llm_calls` | 段・ルーター・要求モデル・**解決モデル**・受け皿の有無・入出力トークン・`cost_usd`・所要時間・ガードレール結果 |
...
- **提案は週 3 件まで**（既存 adr-hygiene の「上限 3」に合わせる）。引用必須
- 過去の `proposal` と重複させない（shared.md）
- **前回の提案が採用された後、次の週に必ず効果を突合する。**
...
| .github/claude-scans/ | `router-retro.md` を追加。`claude-scan.yml` に週次 schedule と matrix を追加。D1 を読む権限（Cloudflare API トークン）を secrets に追加 |
| design-review.md・research-trend.md | **有効 ADR が「0001 / 0002 / 0003」のまま**で ADR-0004 が抜けている。本 ADR の追加と合わせて 0001〜0005 に直す |

## 却下

- **Named Router を使わず BFF のコードでモデルを直指定する**（前版の決定。モデル ID がコードに残り、
  差し替えにデプロイが要る。資料 13 の「モデル選択をコードから追い出す」に反する。**本版で撤回**）
...
- **SDK の再試行を既定のまま使う**（`max_retries=2` がルーターのフェイルオーバーより先に走る。資料 12）
...
- **ローカルの保存率・既読率をクラウドへ送って教師信号にする**（`C-01`。ADR-0004 が書き戻しを却下済み。
  opt-in の可否は未決に回す）
- **内省を日次で回す**（母数が足りず、ノイズを設計変更の根拠にする）
