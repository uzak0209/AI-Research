...
- `docs/requirements/` — 要件。**正本は `00-goals.md` の成功条件 1〜6**
- `docs/adr/` — アーキテクチャ決定（ゴールをどう満たすか）
- `README.md` — 概要
- `.github/` — 自動化ワークフロー
- `prototypes/` — 検証用の使い捨てコード（本番実装ではない）
...
- 機能・制約・成功条件は `docs/requirements/` を正とする（`FR-xx` / `C-xx` / `NFR-xx`）
- 設計判断は `docs/adr/` に ADR として記録する
- 作業前に該当する要件を確認する。要件に無い機能を ADR や実装から生やさない
- 要件を変えるときは先に `docs/requirements/` を更新する。ADR だけ書き換えて要件を黙って変えない
- 技術スタック名は原則 ADR 側。例外は、利用者が選ぶ外部ツールがプロダクト条件のときだけ
  （例: Zotero — `FR-05`）
...
これらは要件の制約（`docs/requirements/02-constraints.md`）を実装・設計で守るためのもの。

**1. 未公開データの恒久的なインデックスを外部に作らない**（`C-01`、ADR-0003, ADR-0012）。

埋め込み生成はローカルモデルのみで行い、埋め込み API は使わない。
ベクトル DB もローカル。クラウドに恒久保存するのは、
公開可能な研究概要・キーワード・論文識別子のみ（ADR-0004）。

**2. LLM 推論は ADR-0010 のデータ分類の統制下でのみ外部へ出す**（`C-04`）。
...
C3 を完全に無効化できる設定を必ず維持する（`C-03`）。

**3. API キーをクライアントに置かない**（`C-06`、ADR-0008）。

OrcaRouter のキーは Workers Secrets にのみ置く。
デスクトップに同梱・埋め込みをしない（必ず抽出される）。

**4. 新機能を追加するときは、必ず該当する要件 ID を確認し、
ADR-0010 のデータ分類を決めてからエンドポイントを作る。**
既存エンドポイントに相乗りさせると分類が崩れる。
